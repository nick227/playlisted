...
describe("visualizerStore", () => {
  ...
  it("does not persist performance disable across sessions", () => {
    const storage = memoryStorage();
    writeVisualizerSettings(storage, {
      ...DEFAULT_VISUALIZER_SETTINGS,
      enabled: false,
    ...
    const saved = JSON.parse(storage.getItem(VISUALIZER_STORAGE_KEY) ?? "{}");
    expect(saved.enabled).toBe(true);
    expect(saved).not.toHaveProperty("disabledReason");
  });

  it("does not persist error disable across sessions", () => {
    const storage = memoryStorage();
    writeVisualizerSettings(storage, {
      ...DEFAULT_VISUALIZER_SETTINGS,
      enabled: false,
