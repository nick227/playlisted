export const GESTURE_EXCLUDE_ATTR = "data-gesture-exclude";

const GESTURE_EXCLUDED_SELECTOR = [
  "a",
  "button",
  "input",
  "select",
  "textarea",
  "[role='button']:not([data-swipe-surface])",
  "[role='link']:not([data-swipe-surface])",
...
export const SWIPE_VELOCITY_THRESHOLD_PX_PER_MS = 0.35;
/** Theatre track-skip is slightly snappier than browse page swipes. */
export const THEATRE_SWIPE_COMMIT_THRESHOLD_PX = 56;
/** Average gesture velocity threshold in CSS pixels per millisecond. */
export const THEATRE_SWIPE_VELOCITY_THRESHOLD_PX_PER_MS = 0.28;
...
export function isGestureExcludedTarget(target: EventTarget | null): boolean {
  if (!(target instanceof Element)) return false;
  return Boolean(target.closest(GESTURE_EXCLUDED_SELECTOR));
}
