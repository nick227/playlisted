...
export type { FigurePose } from '../core/types'
...
export function drawFigure(
  ...
  applyAlpha(ctx, options.alpha)
  const s = scale * (1.15 + energy * 0.08)
...
export function drawFaceMask(
  ...
  ctx.lineWidth = 1.5
  ctx.beginPath()
  ctx.ellipse(x, y, s * 1.05, s * 1.2, 0, 0, Math.PI * 2)
  ctx.fill()
  ctx.stroke()
  const eyeY = y - s * 0.15
  const eyeOff = s * 0.28
  ...
  for (const side of [-1, 1]) {
    ...
    ctx.beginPath()
    ctx.ellipse(ex, ey, s * 0.14, s * 0.095, 0, 0, Math.PI * 2)
    ctx.fill()
    ctx.fillStyle = palette.figure
    ctx.beginPath()
  ...
  ctx.fillStyle = palette.magenta
  ctx.fillRect(x - s * 0.24, y + s * 0.28, s * 0.48, mouthH)
  resetAlpha(ctx, options.alpha)
}
