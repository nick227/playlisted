...
export function registerAnimationPackage(pkg: AnimationPackage) {
  ...
  if (!Array.isArray(pkg.animations) || pkg.animations.length === 0) {
    throw new Error(`[Theatre package:${packageId}] packages must register at least one animation`)
  }
  if (!Array.isArray(pkg.presets) || pkg.presets.length === 0) {
    throw new Error(`[Theatre package:${packageId}] packages must register at least one preset`)
  }
